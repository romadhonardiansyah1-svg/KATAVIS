...
Klien melakukan `PUT` biner langsung ke `uploadUrl`. Worker tidak pernah menyalurkan berkas —
batas Workers tidak memungkinkannya untuk berkas besar.
...
|---|---|---|---|---|
| `UNAUTHENTICATED` | 401 | `LOGIN` | true | Sesi Anda sudah berakhir. Silakan masuk lagi. |
| `FORBIDDEN` | 403 | `NONE` | true | Anda tidak punya akses untuk tindakan ini. |
| `NOT_FOUND` | 404 | `GO_BACK` | true | Halaman tidak ditemukan. |
| `RATE_LIMITED` | 429 | `WAIT` | true | Terlalu banyak percobaan. Coba lagi beberapa menit lagi. |
| `ACCOUNT_LOCKED` | 423 | `WAIT` | true | Akun terkunci sementara. Coba lagi 15 menit lagi. |
| `CONSENT_REQUIRED` | 403 | `GIVE_CONSENT` | true | Perlu persetujuan Anda sebelum melanjutkan. |
| `FILE_TOO_LARGE` | 413 | `PICK_OTHER_FILE` | true | Foto terlalu besar. Maksimal 10 MB. |
| `UNSUPPORTED_FORMAT` | 415 | `PICK_OTHER_FILE` | true | Format foto tidak didukung. Gunakan JPG atau PNG. |
| `CONTENT_MISMATCH` | 400 | `PICK_OTHER_FILE` | true | Berkas ini bukan foto. Silakan pilih foto lain. |
| `ASR_TOO_SHORT` | 400 | `RETRY_RECORD` | true | Rekaman terlalu pendek. Ceritakan sekitar 30 detik. |
| `ASR_TOO_LONG` | 400 | `RETRY_RECORD` | true | Rekaman terlalu panjang. Maksimal 60 detik. |
| `ASR_NO_SPEECH` | 422 | `RETRY_RECORD` | true | Suara belum terdengar jelas. Rekam lagi di tempat lebih tenang. |
| `TRANSCRIPT_NOT_REVIEWED` | 409 | `REVIEW_TRANSCRIPT` | true | Periksa dulu hasil transkrip Anda. |
| `CONTENT_INCOMPLETE` | 409 | `COMPLETE_CONTENT` | true | Katalog belum lengkap. Lengkapi dulu nama dan cerita produk. |
| `PHOTO_REQUIRED` | 409 | `ADD_PHOTO` | true | Tambahkan minimal satu foto produk. |
| `IMAGE_GENERATE_FAILED` | 502 | `RETRY_OR_USE_ORIGINAL` | **true** | Foto studio belum berhasil dibuat. Foto asli Anda tetap tersimpan. |
| `COPY_GENERATE_FAILED` | 502 | `RETRY` | **true** | Cerita belum berhasil dibuat. Rekaman Anda tetap tersimpan. |
| `MAX_RETRIES_EXCEEDED` | 429 | `USE_ORIGINAL` | true | Sudah dicoba beberapa kali. Anda bisa memakai foto asli. |
| `QUOTA_EXCEEDED` | 503 | `WAIT` | true | Sistem sedang sibuk. Pekerjaan Anda tersimpan. |
| `NETWORK_OFFLINE` | — | `WAIT_ONLINE` | true | Tidak ada koneksi. Pekerjaan Anda tersimpan dan akan dilanjutkan. |
| `INVITE_EXPIRED` | 410 | `REQUEST_NEW_INVITE` | true | Undangan sudah kedaluwarsa. Minta undangan baru. |
| `INVITE_ALREADY_USED` | 409 | `NONE` | true | Undangan ini sudah dipakai. |
| `INTERNAL_ERROR` | 500 | `RETRY` | true | Terjadi gangguan. Pekerjaan Anda tersimpan. Coba lagi. |
