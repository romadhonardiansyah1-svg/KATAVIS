...
**Sumber naskah berwaktu.** `narration.captions` diturunkan dari `story` oleh
`worker/catalog/narration.ts`: cerita dibagi menjadi kalimat, dan tiap kalimat diberi waktu
menurut laju baca 150 kata/menit dengan jeda 320 ms di antaranya. Ini tidak memerlukan penyedia
TTS dan tidak memerlukan jaringan, sehingga fitur ini hidup tanpa menunggu keputusan O4 di
ADR-005.

`audioUrl` tetap `null` sampai penyedia TTS diputuskan. Yang berjalan tanpa audio bukan pemutar
kosong: subjudul, jam, penyorotan kalimat, dan seluruh cerita sebagai teks semuanya bekerja, dan
F3-03 memang menuntut hal itu. Bila TTS ditambahkan kelak, yang berubah hanya `audioUrl` —
naskah berwaktunya sudah menunggu.
...
Kanal audio dan getar tinggal di `lib/notify.ts`; kanal visualnya di `StepShell` dan di setiap
halaman. Nada dibangkitkan `OscillatorNode`, bukan berkas di `public/`, sehingga S10-02 terjamin
secara struktural: nada sukses menaik, nada gagal menurun dan lebih rendah. Peristiwa "Katalog
selesai" memakai kalimat yang diucapkan, bukan nada — nada tidak memberi tahu pengrajin tunanetra
bahwa katalognya terbit.
