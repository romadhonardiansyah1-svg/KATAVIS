...
Skema lengkap dan tervalidasi ada di `migrations/0001_initial_schema.sql` — **11 tabel** — dan
`migrations/0002_auth.sql` — **2 tabel tambahan**. Bagian di
...
### Dua tabel autentikasi (migrasi 0002)

| Tabel | Peran | Menegakkan |
|---|---|---|
| `otp_codes` | Satu kode aktif per nomor, disimpan sebagai SHA-256 | TC-SEC-14 — permintaan baru menimpa yang lama |
| `auth_rate_limits` | Satu baris per kejadian, jendela dihitung dari waktu | TC-SEC-12 — 3 per nomor dan 10 per IP per jam |

Keduanya ada karena batas laju **tidak boleh** hidup di memori Worker: isolat dapat dimatikan
kapan saja dan hitungannya hilang bersama prosesnya. `users` juga menerima dua kolom baru,
`pin_attempts` dan `pin_locked_until`, yang menyimpan penguncian 15 menit setelah 5 percobaan
gagal (TC-SEC-13).
