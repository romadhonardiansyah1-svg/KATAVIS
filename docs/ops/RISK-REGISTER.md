...
4. **Mematikan agennya sendiri**, bukan lewat bendera di Worker. Hentikan proses Studio Agent dan
   seluruh rantai Gemini-nya hilang dalam hitungan detik: tanpa denyut, rantai penyedia
   melewatinya sendiri (`worker/jobs/d1-agent.ts`).

   *Koreksi:* butir ini dulu berbunyi "feature flag untuk mematikan jalur Gemini sepenuhnya".
   Bendera itu ada namanya (`GEMINI_ENABLED`) tetapi **tidak pernah dibaca satu baris pun kode**,
   sementara `DEMO-RUNBOOK` menyuruh operator memeriksanya. Mitigasi yang dipercaya tetapi tidak
   terhubung ke apa pun lebih berbahaya daripada tidak ada mitigasi: saat selector Gemini pecah
   di panggung, yang menekannya akan menekan sakelar mati. Benderanya dihapus; yang tersisa
   adalah cara yang benar-benar bekerja.
